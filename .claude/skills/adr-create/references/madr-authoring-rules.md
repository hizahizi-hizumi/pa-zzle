...
MADR v3 の章立てを日本語化して採用する。ADR は判断履歴であり、現在仕様の正本ではない。決定によって現在仕様が変わる場合は、関連するコードや文書も別途更新する。
...
| `## 要約` | 必須 | `決定` / `主な選択肢` / `背景` / `要約` の4項目。最後の `要約` は `docs/adr/index.csv` の `summary` と完全一致させる |
...
`docs/adr/ADR-<NNNN>-<ascii-slug>.md`
...
`docs/adr/index.csv` は6カラム: `id,title,summary,status,date,path`。
...
```csv
ADR-0001,API境界をHTTPで統一,"フロントエンドとバックエンドの境界をHTTP APIに統一する。直接呼び出しとHTTP APIを比較した。実行環境を分離しても同じ契約を維持できることを優先した。",proposed,2026-09-13,docs/adr/ADR-0001-http-api-boundary.md
```
