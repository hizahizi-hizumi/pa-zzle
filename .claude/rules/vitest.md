...
- ケース名に条件を含む場合は `describe("<条件>の場合", ...)` に分割し、その中のケース名は期待動作と、それを引き起こす操作だけを記述する。
  - 条件ごとのArrangeを `describe` 側へ置き、同じ条件を持つケースをまとめるため。
  - 条件は、前提となる状態や、操作・入力の対象が持つ性質など、Arrangeで決まるものを指す。操作そのものは条件に含めない。
  - `test.each` のケース名は「入力違いのケース」に従う。
