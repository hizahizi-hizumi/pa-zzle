...
export type RegionAnchorBenchmark = {
  origin: string;
  rules: BenchmarkRule[];
  cases: BenchmarkCase[];
};

export async function loadRegionAnchorBenchmark(
  benchmarkPath: string,
): Promise<RegionAnchorBenchmark> {
  const text = await Bun.file(benchmarkPath).text();
  return compileRegionAnchorBenchmark(YAML.parse(text), benchmarkPath);
}

export function compileRegionAnchorBenchmark(
  value: unknown,
  origin: string,
  ...
  if (
    ...
    throw new Error(`region-anchor benchmarkが不正です: ${origin}`);
  }
