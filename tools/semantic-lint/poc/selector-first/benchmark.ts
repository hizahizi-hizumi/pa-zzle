...
export type SelectorFirstBenchmark = {
  origin: string;
  rules: BenchmarkRule[];
  cases: BenchmarkCase[];
};

export async function loadSelectorFirstBenchmark(
  benchmarkPath: string,
): Promise<SelectorFirstBenchmark> {
  const text = await Bun.file(benchmarkPath).text();
  return compileSelectorFirstBenchmark(YAML.parse(text), benchmarkPath);
}

export function compileSelectorFirstBenchmark(
  value: unknown,
  origin: string,
  ...
  if (
    ...
    throw new Error(`selector-first benchmarkが不正です: ${origin}`);
  }

  const rules = value.rules.map((rule, index) => compileRule(rule, origin, index));
...
function compileRule(value: unknown, origin: string, index: number): BenchmarkRule {
  if (!isRecord(value)) {
  ...
  return {
    ...
    title,
    violationThreshold,
    predicate: { instruction: predicate.instruction, outcomes },
  };
}
