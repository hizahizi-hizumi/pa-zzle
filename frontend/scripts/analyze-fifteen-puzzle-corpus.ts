import {
  ...
  FIFTEEN_PUZZLE_MINIMUM_OPTIMAL_MOVE_COUNT,
  fifteenPuzzleDifficulties,
} from "@/games/fifteen-puzzle/difficulty";
import {
  analyzeFifteenPuzzleDifficulty,
  type FifteenPuzzleDifficultyFeatures,
} from "@/games/fifteen-puzzle/problem/difficulty-analysis";
import {
...
type Sample = FifteenPuzzleDifficultyFeatures & {
  seed: string;
...
function isGreedyPlayCompleted(
  ...
  let distance = calculateFifteenPuzzleManhattanDistance(board);
  while (distance > 0) {
    const nextBoards = listFifteenPuzzleSingleMoves(board).flatMap(
      (tileIndex) => {
        const slide = getFifteenPuzzleSlide(board, tileIndex);
        const nextBoard = slide ? applyFifteenPuzzleSlide(board, slide) : null;
        return nextBoard &&
          calculateFifteenPuzzleManhattanDistance(nextBoard) < distance
          ? [nextBoard]
          : [];
      },
    );
...
function levelOf(sample: FifteenPuzzleDifficultyFeatures) {
  return assessFifteenPuzzleDifficulty({
    status: "analyzed",
    features: sample,
  });
}

function formatBoard(board: FifteenPuzzleBoard): string {
  return Array.from({ length: FIFTEEN_PUZZLE_SIZE }, (_, row) =>
    board
      .slice(row * FIFTEEN_PUZZLE_SIZE, (row + 1) * FIFTEEN_PUZZLE_SIZE)
...
function analyzeCorpus(perScramble: number): Sample[] {
  ...
  for (const scrambleLength of corpusScrambleLengths) {
    for (let index = 0; index < perScramble; index += 1) {
      ...
      const board = generateFifteenPuzzleBoard(seed, {
        size: FIFTEEN_PUZZLE_SIZE,
        scrambleLength,
      });
      const startedAt = performance.now();
      const solved = solveFifteenPuzzleOptimally(board, { heuristic });
...
function printCorpusReport(samples: readonly Sample[]) {
  ...
  const provided = samples.filter(
    (sample) =>
      sample.optimalMoveCount >= FIFTEEN_PUZZLE_MINIMUM_OPTIMAL_MOVE_COUNT,
  );
  console.log(
  ...
  printTable(
    [
      ...
      "問題数",
      "遠回り （最小 / 25% / 中央 / 75% / 最大）",
      ...fifteenPuzzleDifficulties.map(({ label }) => label),
    ],
    bands.map((band) => {
      ...
      return [
        `${Math.max(band, FIFTEEN_PUZZLE_MINIMUM_OPTIMAL_MOVE_COUNT)}〜${band + 4}`,
        String(group.length),
        summarize(group.map((sample) => sample.detourMoveCount)),
        ...fifteenPuzzleDifficulties.map(({ id }) =>
  ...
  const levelCounts = fifteenPuzzleDifficulties.map(
...
function printPoolReport() {
  const levels = fifteenPuzzleDifficulties.map(({ id }) => ({
    id,
    samples: listFifteenPuzzlePoolEntries(id).map((entry) => {
      ...
      const analysis = analyzeFifteenPuzzleDifficulty(board, optimalMoveCount);
      if (analysis.status !== "analyzed") {
  ...
  for (const { id } of fifteenPuzzleDifficulties) {
