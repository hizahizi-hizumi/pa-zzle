...
import { FIFTEEN_PUZZLE_GENERATOR_VERSION } from "@/games/fifteen-puzzle/problem/problem";
import type { FifteenPuzzleProblemPoolEntry } from "@/games/fifteen-puzzle/problem/problem-pool";
import { FIFTEEN_PUZZLE_SIZE } from "@/games/fifteen-puzzle/puzzle/state";

const outputPath = new URL(
  "../src/games/fifteen-puzzle/problem/problem-pool.json",
  import.meta.url,
);
...
type Candidate = {
  entry: FifteenPuzzleProblemPoolEntry;
  boardKey: string;
  level: FifteenPuzzleDifficulty;
...
function levelsUsing(scrambleLength: number): FifteenPuzzleDifficulty[] {
  return fifteenPuzzleDifficulties
    .map(({ id }) => id)
    .filter((id) =>
      fifteenPuzzleDifficultyCriteria[id].scrambleLengths.includes(
        scrambleLength,
      ),
    );
}
...
function runMain() {
  ...
  const scrambleLengths = [
    ...new Set(
      fifteenPuzzleDifficulties.flatMap(
        ({ id }) => fifteenPuzzleDifficultyCriteria[id].scrambleLengths,
      ),
    ),
  ...
  for (const scrambleLength of scrambleLengths) {
    ...
    for (
      ...
      const level = assessFifteenPuzzleDifficulty(analysis);
  ...
  const levels = Object.fromEntries(
    fifteenPuzzleDifficulties.map(({ id }) => {
      const perProfile = fifteenPuzzleDifficultyCriteria[
        id
      ].scrambleLengths.map(
        (scrambleLength) =>
  ...
  ) as Record<FifteenPuzzleDifficulty, Candidate[]>;

  const lines = fifteenPuzzleDifficulties.map(
    ({ id }, levelIndex) =>
      `    ${JSON.stringify(id)}: [\n${levels[id]
        .map(({ entry }) => `      ${JSON.stringify(entry)}`)
        .join(",\n")}\n    ]${
        levelIndex < fifteenPuzzleDifficulties.length - 1 ? "," : ""
      }`,
  );
  writeFileSync(
    outputPath,
    `{\n  "generatorVersion": ${JSON.stringify(FIFTEEN_PUZZLE_GENERATOR_VERSION)},\n  "levels": {\n${lines.join("\n")}\n  }\n}\n`,
  );
