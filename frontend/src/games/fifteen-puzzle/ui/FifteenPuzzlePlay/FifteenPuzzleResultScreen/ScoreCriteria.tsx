...
import {
  ...
  FIFTEEN_PUZZLE_SPEED_PER_OPTIMAL_MOVE_MS,
    ...
    <dl className="mt-3 grid gap-3 text-meta">
      <div>
        ...
        <dd className="mt-1">
          ...
          {FIFTEEN_PUZZLE_SCORE_MAXIMUMS.efficiency}点。
          総手数に対する最短手数の割合で配点し、{efficiencyHalfMoveCount}
          手で{FIFTEEN_PUZZLE_SCORE_MAXIMUMS.efficiency / 2}
      ...
      <div>
        ...
        <dd className="mt-1">
          ...
          {FIFTEEN_PUZZLE_SCORE_MAXIMUMS.speed}点。
          {formatScoreTime(speedZeroScoreMs)}
          以上で0点、その間は時間に応じて減点。基準時間は
          {`${FIFTEEN_PUZZLE_SPEED_INITIAL_RECOGNITION_MS / 1000}秒 + 最短${result.optimalMoveCount}手 × ${FIFTEEN_PUZZLE_SPEED_PER_OPTIMAL_MOVE_MS / 1000}秒`}
          。
        </dd>
      </div>
      <div>
