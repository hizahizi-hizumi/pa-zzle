import { createFifteenPuzzlePlayRecord } from "@/games/fifteen-puzzle/play-record";
...
function getFormattedMetric(metricId: string): string | null {
  const metricDisplay = fifteenPuzzlePlayRecordDisplay.metrics.find(
    (metric) => metric.id === metricId,
  );
  const value = getPlayRecordMetricValue(
    record,
    fifteenPuzzlePlayRecordDisplay.definition,
    metricId,
  );
...
describe("fifteenPuzzlePlayRecordDisplay", () => {
  ...
  test.each(comparisonLabelCases)(
    ...
    (key, expected) => {
      const label = fifteenPuzzlePlayRecordDisplay.getComparisonLabel(key);
