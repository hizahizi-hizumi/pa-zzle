  ...
  async function copySnapshot() {
    try {
      await navigator.clipboard.writeText(
        serializeNanpureDiagnosticSnapshot(snapshot),
      );
      setCopyState("copied");
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center sm:p-6">
      ...
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="nanpure-diagnostics-title"
      ...
      >
        ...
        <dl className="mt-5 divide-y rounded-xl border bg-muted/25 px-4">
          <DiagnosticRow
            label="難易度"
            value={getNanpureDifficultyLabel(snapshot.difficulty)}
          />
          <DiagnosticRow
            label="seed"
          ...
          <DiagnosticRow
            label="生成条件"
            value={`ヒント ${snapshot.problemIdentity.conditions.clueCount}`}
          />
          <DiagnosticRow
            label="生成試行"
