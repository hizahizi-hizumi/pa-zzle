...
describe("traceNanpureHumanSolve", () => {
  test("基礎手筋だけで解ける問題を最後まで解くこと", () => {
    const result = traceNanpureHumanSolve(basicPuzzle);
  ...
  test("対応済み手筋で進めなくなった問題を推測せず停止すること", () => {
    const classification = classifyNanpureSolutions(advancedPuzzle);
    const result = traceNanpureHumanSolve(advancedPuzzle);
  ...
  test("同じ盤面から同じ解法トレースを再現すること", () => {
    const first = traceNanpureHumanSolve(basicPuzzle);
    const second = traceNanpureHumanSolve(basicPuzzle);
  ...
  test("Hidden Singleの成立単位と候補位置を記録すること", () => {
    const result = traceNanpureHumanSolve(hiddenSinglePuzzle);
    const firstStep = result.steps[0];
