...
const legacyRecordWithoutRestart: PlayRecord = {
...
const legacyRecordWithRestart: PlayRecord = {
...
test("やり直しのない旧記録は待った回数からクリア手数を復元して再評価すること", () => {
  const completionMoveCount = getWaterSortPlayRecordCompletionMoveCount(
    legacyRecordWithoutRestart,
  );
  const score = getWaterSortPlayRecordScore(legacyRecordWithoutRestart);
  const isRecord = isWaterSortPlayRecord(legacyRecordWithoutRestart);
...
test("やり直しで失われた手数を復元できない旧記録は評価を推測しないこと", () => {
  const score = getWaterSortPlayRecordScore(legacyRecordWithRestart);
  const metricValue = scoreMetric?.getValue(legacyRecordWithRestart);
  const isRecord = isWaterSortPlayRecord(legacyRecordWithRestart);
