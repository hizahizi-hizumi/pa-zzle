...
describe("restartWaterSortSession", () => {
  test("プレイ中は同じ計測を継続して初期盤面へ戻すこと", () => {
    ...
    const result = restartWaterSortSession(moved, 2000);
  ...
  test("クリア後は同じ問題を新しいプレイとして開始すること", () => {
    const problem = createProblem();
    ...
    const result = restartWaterSortSession(session, 5000);
...
test("Reactなしで操作・待った・やり直しを経て一局を完結できること", () => {
  ...
  session = restartWaterSortSession(session, 1200);
  for (const [index, move] of problem.solutionMoves.entries()) {
  ...
  expect(result).toEqual({
    ...
    undoCount: 1,
    restartCount: 1,
    optimalMoveCount: problem.solutionMoves.length,
    moveDelta: 1,
