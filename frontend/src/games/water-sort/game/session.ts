import type { WaterSortProblem } from "./generator";
...
export function restartWaterSortSession(
  ...
  if (session.status === "cleared") {
    return createWaterSortSession(session.problem, restartedAt);
  }
...
export function getWaterSortSessionResult(
  ...
  return {
    ...
    optimalMoveCount,
    moveDelta: session.moveCount - optimalMoveCount,
    score: calculateWaterSortPlayScore(session.moveCount, optimalMoveCount),
  };
}
