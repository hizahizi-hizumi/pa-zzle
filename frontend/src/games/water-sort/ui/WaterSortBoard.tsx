...
type BottleRect = { left: number; top: number; width: number; height: number };
...
export function WaterSortBoard({
  ...
  const [pourPresentations, setPourPresentations] = useState<readonly PourPresentation[]>([]);
  ...
  useEffect(() => {
    ...
    const destinationBottle = bottleRefs.current[operation.destinationBottleIndex];
    ...
    const presentation: PourPresentation = {
      ...
      destinationBefore: operation.stateBefore[operation.destinationBottleIndex] ?? [],
      destinationAfter: operation.stateAfter[operation.destinationBottleIndex] ?? [],
      pourColorIndex,
    ...
    setPourPresentations((current) => [
      ...current.filter(
        (activePresentation) =>
          activePresentation.sourceBottleIndex !== operation.sourceBottleIndex &&
          activePresentation.destinationBottleIndex !== operation.sourceBottleIndex &&
          activePresentation.sourceBottleIndex !== operation.destinationBottleIndex &&
          activePresentation.destinationBottleIndex !== operation.destinationBottleIndex,
      ),
      presentation,
    ]);
    ...
    <>
      ...
      >
        {state.map((bottle, bottleIndex) => {
          ...
          const contents = bottle.length === 0 ? "空" : bottle.map((colorIndex) => getColorView(colorIndex).label).join("、");
          const isSource = bottleIndex === sourceBottleIndex;
          const isAnimated = animatedBottleIndexes.has(bottleIndex);
          return (
            <button
              key={bottleLabel}
              ref={(element) => { bottleRefs.current[bottleIndex] = element; }}
              type="button"
...
function PourPresentationLayer({ presentation, onFinish, onClearingPourComplete }: { presentation: PourPresentation; onFinish: (presentationId: number) => void; onClearingPourComplete?: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!sourceElement?.animate) {
      ...
      if (presentation.isClearingMove) onClearingPourComplete?.();
      return;
    }
    ...
    const sourceAnimation = sourceElement.animate([
      ...
      { transform: "translate(0, -4px) rotate(0deg)", offset: 0.06, easing: "cubic-bezier(.22,.61,.36,1)" },
      { transform: `translate(${deltaX}px, ${hoverY}px) rotate(0deg)`, offset: 0.2, easing: "ease-in-out" },
      { transform: `translate(${deltaX}px, ${hoverY}px) rotate(${direction * 68}deg)`, offset: 0.32, easing: "ease-out" },
      { transform: `translate(${deltaX}px, ${hoverY}px) rotate(${direction * 68}deg)`, offset: 0.76, easing: "ease-in" },
      { transform: `translate(${deltaX}px, ${hoverY}px) rotate(0deg)`, offset: 0.86, easing: "cubic-bezier(.22,.61,.36,1)" },
    ...
    const streamAnimation = streamElement?.animate?.([
      ...
      { opacity: 0.9, transform: "scaleY(1)", offset: pourTransferStartOffset },
    ...
    const sourceTransferAnimation = sourceTransferElement?.animate?.([
      { transform: "scaleY(1)", offset: 0 }, { transform: "scaleY(1)", offset: pourTransferStartOffset }, { transform: "scaleY(0)", offset: pourTransferEndOffset }, { transform: "scaleY(0)", offset: 1 },
    ...
    const destinationTransferAnimation = destinationTransferElement?.animate?.([
      { transform: "scaleY(0)", offset: 0 }, { transform: "scaleY(0)", offset: pourTransferStartOffset }, { transform: "scaleY(1)", offset: pourTransferEndOffset }, { transform: "scaleY(1)", offset: 1 },
    ], { duration: pourAnimationDurationMs, easing: "linear", fill: "forwards" });
  ...
  return createPortal(<>
    <div ref={sourceRef} aria-hidden="true" className="group pointer-events-none fixed aspect-[0.36] origin-top rounded-b-[1.45rem] will-change-transform" style={getOverlayStyle(presentation.sourceRect, sourcePourLayerZIndex)}>
      <BottleVisual bottle={presentation.sourceAfter} transfer={{ colorIndex: presentation.pourColorIndex, startSlot: presentation.sourceAfter.length, slotCount: presentation.sourceBefore.length - presentation.sourceAfter.length, elementRef: sourceTransferRef, initialScaleY: 1 }} />
    </div>
    <div ref={streamRef} aria-hidden="true" className="pointer-events-none fixed origin-top rounded-full opacity-0 will-change-transform" style={getPourStreamStyle(presentation)} />
    <div ref={destinationRef} aria-hidden="true" className="group pointer-events-none fixed aspect-[0.36] rounded-b-[1.45rem]" style={getOverlayStyle(presentation.destinationRect, destinationPourLayerZIndex)}>
      <BottleVisual bottle={presentation.destinationBefore} transfer={{ colorIndex: presentation.pourColorIndex, startSlot: presentation.destinationBefore.length, slotCount: presentation.destinationAfter.length - presentation.destinationBefore.length, elementRef: destinationTransferRef, initialScaleY: 0 }} />
    </div>
  </>, document.body);
}

type TransferLiquid = { colorIndex: number; startSlot: number; slotCount: number; elementRef: RefObject<HTMLSpanElement | null>; initialScaleY: number };
function BottleVisual({ bottle, transfer }: { bottle: WaterSortBottle; transfer?: TransferLiquid }) {
  return <>
    <span aria-hidden="true" className="absolute inset-x-[4px] bottom-[4px] top-3 overflow-hidden rounded-b-[1.15rem] bg-black/[0.015]">
      {bottleSlots.map((slotIndex) => {
        ...
        return <span key={`${slotIndex}-${colorIndex}-${bottle.length}`} className="absolute inset-x-0 h-1/4 transition-[background-color] duration-200" style={{ bottom: `${slotIndex * 25}%`, backgroundColor: color.color }} />;
      })}
      {transfer ? <span ref={transfer.elementRef} className="absolute inset-x-0 origin-bottom will-change-transform" style={{ bottom: `${transfer.startSlot * 25}%`, height: `${transfer.slotCount * 25}%`, backgroundColor: getColorView(transfer.colorIndex).color, transform: `scaleY(${transfer.initialScaleY})` }} /> : null}
    </span>
    <span aria-hidden="true" className="absolute inset-x-0 bottom-0 top-2 rounded-b-[1.45rem] border-[3px] border-t-0 border-slate-400/55 shadow-[inset_0_-2px_5px_rgba(15,23,42,0.08),0_5px_12px_rgba(15,23,42,0.06)] transition-[border-color,filter] duration-150 group-data-[selected=true]:border-slate-500 group-data-[selected=true]:drop-shadow-md" />
    <span aria-hidden="true" className="absolute left-1/2 top-0 h-[3px] w-[calc(100%-2px)] -translate-x-1/2 rounded-full bg-slate-400/55" />
  </>;
}

function getBoardLayout(bottleCount: number) {
  const columnCount = Math.max(3, Math.ceil(bottleCount / 2));
  if (columnCount >= 6) return { columnCount, width: "min(100%, clamp(366px, 48vw, 570px))", columnGap: 10, rowGap: 34 };
  if (columnCount === 5) return { columnCount, width: "min(100%, clamp(360px, 44vw, 520px))", columnGap: 14, rowGap: 40 };
  if (columnCount === 4) return { columnCount, width: "min(100%, clamp(320px, 40vw, 450px))", columnGap: 16, rowGap: 42 };
  return { columnCount, width: "min(100%, clamp(250px, 32vw, 340px))", columnGap: 18, rowGap: 44 };
}
function animateInvalidBottle(element: HTMLButtonElement | null | undefined) {
  if (!element?.animate) return;
  element.animate([{ transform: "translateX(0)" }, { transform: "translateX(-4px)" }, { transform: "translateX(4px)" }, { transform: "translateX(-2px)" }, { transform: "translateX(0)" }], { duration: 220, easing: "ease-out" });
}
function captureRect(element: HTMLElement): BottleRect { const rect = element.getBoundingClientRect(); return { left: rect.left, top: rect.top, width: rect.width, height: rect.height }; }
function getOverlayStyle(rect: BottleRect, zIndex: number) { return { left: `${rect.left}px`, top: `${rect.top}px`, width: `${rect.width}px`, height: `${rect.height}px`, zIndex }; }
function getPourStreamStyle(presentation: PourPresentation) {
  ...
  return { left: `${destinationRect.left + destinationRect.width / 2 - 2}px`, top: `${streamTop + 3}px`, width: "4px", height: `${streamHeight}px`, backgroundColor: color, boxShadow: `0 0 5px ${color}66`, zIndex: streamPourLayerZIndex };
}
function getColorView(colorIndex: number) {
  const color = waterSortColors[colorIndex];
  if (!color) return { label: `色 ${colorIndex + 1}`, color: `hsl(${(colorIndex * 47) % 360} 70% 58%)` };
  return color;
}
