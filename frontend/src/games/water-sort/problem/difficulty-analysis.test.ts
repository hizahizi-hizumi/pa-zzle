...
describe("analyzeWaterSortDifficulty", () => {
  ...
  const limitedRiskOptions = { maxExpandedStatesPerRiskChoice: 0 } as const;
  const singleRiskChoiceOptions = { maximumRiskChoices: 1 } as const;
  ...
  test("追加探索の上限へ達した選択肢を未解決として残すこと", () => {
    const analysis = analyzeWaterSortDifficulty(
      initialState,
      solved.moves,
      limitedRiskOptions,
    );
