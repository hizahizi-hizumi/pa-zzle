...
type HookResultRef = { current: HookResult };
...
function solveCurrentProblem(result: HookResultRef) {
  for (const move of solutionMoves) {
    performMove(result, move);
  }
}

function getFirstLegalMove(state: WaterSortState): WaterSortMove {
  const move = listWaterSortLegalMoves(state)[0];
  if (!move) {
...
function getIllegalDestination(
  state: WaterSortState,
  sourceBottleIndex: number,
): number {
  const legalDestinations = new Set(
    listWaterSortLegalMoves(state)
      .filter((move) => move.sourceBottleIndex === sourceBottleIndex)
      .map((move) => move.destinationBottleIndex),
  );
  const destination = state.findIndex(
    (_, bottleIndex) =>
      bottleIndex !== sourceBottleIndex && !legalDestinations.has(bottleIndex),
  );
  if (destination < 0) {
...
describe("useWaterSortPlay", () => {
  describe.each(difficultyCases)("%s をプレイする場合", (difficulty) => {
    let result: HookResultRef;
    ...
    test("特徴量条件を満たす実盤面と最短手数をプレイ開始時から保持すること", () => {
      const emptyBottles = result.current.state.filter(
        (bottle) => bottle.length === 0,
      );

      expect(emptyBottles).toHaveLength(2);
  ...
  describe("easy をプレイする場合", () => {
    ...
    describe("合法な注ぎ元と注ぎ先を選べる場合", () => {
      let move: WaterSortMove;
      let expected: WaterSortState | null;

      beforeEach(() => {
        move = getFirstLegalMove(result.current.state);
        expected = applyWaterSortMove(result.current.state, move);
      });

      test("パズルの状態遷移を適用すること", () => {
        performMove(result, move);
    ...
    describe("注ぎ元を選択済みで不正な注ぎ先を選ぶ場合", () => {
      let initialState: WaterSortState;
      let legalMove: WaterSortMove;
      let illegalDestination: number;

      beforeEach(() => {
        initialState = result.current.state;
        legalMove = getFirstLegalMove(initialState);
        illegalDestination = getIllegalDestination(
          initialState,
    ...
    describe("成立済みの注水履歴がある場合", () => {
      ...
      beforeEach(() => {
        initialState = result.current.state;
        performMove(result, getFirstLegalMove(initialState));
      });
    ...
    describe("元に戻せる履歴がない場合", () => {
      test("元に戻す操作を回数へ含めないこと", () => {
        act(() => result.current.undo());
    ...
    describe("手を進めて待ったとやり直しを行った場合", () => {
      ...
      beforeEach(() => {
        ...
        performMove(result, getFirstLegalMove(result.current.state));
        act(() => result.current.undo());
  ...
  describe("プレイ中に時間が経過している場合", () => {
    ...
    test("やり直すと同じ問題へ戻し手数と経過時間を累積すること", () => {
      act(() => result.current.restart());
