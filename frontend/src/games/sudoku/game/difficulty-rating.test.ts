...
import {
  classifySudokuDependency,
  rateUniqueSudokuDifficulty,
  SUDOKU_DIFFICULTY_MODEL_VERSION,
} from "./difficulty-rating";
import type { SudokuBoard, SudokuCell } from "./state";

function boardFromString(value: string): SudokuBoard {
  return [...value].map<SudokuCell>((cell) =>
    cell === "." || cell === "0" ? null : (Number(cell) as SudokuCell),
  );
}
...
describe("classifySudokuDependency", () => {
  test.each([
    [14.6, "easy"],
    [14.59, "normal"],
    [9.37, "normal"],
    ...
    (meanAvailablePlacementCount, difficulty) => {
      const result = classifySudokuDependency({
...
describe("rateUniqueSudokuDifficulty", () => {
  test.each([
    ["easy", easyPuzzle],
    ["normal", normalPuzzle],
    ["hard", hardPuzzle],
  ] as const)("%s の代表問題を分類できること", (difficulty, puzzle) => {
    const result = rateUniqueSudokuDifficulty(puzzle);
  ...
  test("対応手筋で解き切れない問題を難しい問題と決めつけないこと", () => {
    const result = rateUniqueSudokuDifficulty(unsupportedPuzzle);
