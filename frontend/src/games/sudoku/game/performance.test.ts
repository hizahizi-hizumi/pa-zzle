...
describe("calculateSudokuPlayScore", () => {
  test("ミスも超過時間も待ったもやり直しもなければ100点になること", () => {
    const score = calculateSudokuPlayScore({
      elapsedMs: SUDOKU_SPEED_FULL_SCORE_MS,
      mistakeCount: 0,
      undoCount: 0,
      restartCount: 0,
    });
  ...
  test("15分を超えた時間を1分単位で切り上げて速さを減点すること", () => {
    const justOver = calculateSudokuPlayScore({
      elapsedMs: SUDOKU_SPEED_FULL_SCORE_MS + 1,
      mistakeCount: 0,
      undoCount: 0,
      restartCount: 0,
    });
    const twoMinutesOver = calculateSudokuPlayScore({
      elapsedMs: SUDOKU_SPEED_FULL_SCORE_MS + 120_000,
      mistakeCount: 0,
      undoCount: 0,
      restartCount: 0,
    });
  ...
  test("各評価軸を0点未満にしないこと", () => {
    const score = calculateSudokuPlayScore({
      elapsedMs: SUDOKU_SPEED_FULL_SCORE_MS + 10_000_000,
      mistakeCount: 100,
      undoCount: 100,
      restartCount: 100,
    });
