...
describe("SudokuPlay", () => {
  ...
  test("クリア後に再挑戦操作を通知すること", async () => {
    const retry = vi.fn();
    await renderSudokuPlay({ status: "cleared", retry });
  ...
  test("難易度変更操作を通知すること", async () => {
    ...
    await renderSudokuPlay({ onChangeDifficulty });
...
async function renderSudokuPlay(
  overrides: Partial<ComponentProps<typeof SudokuPlay>> = {},
) {
  const container = document.createElement("div");
  document.body.append(container);
  reactRoot = createRoot(container);
  const props: ComponentProps<typeof SudokuPlay> = {
    difficulty: "normal",
    seed: "test-seed",
    status: "playing",
    clear: vi.fn(),
  ...
  await act(async () => reactRoot?.render(<SudokuPlay {...props} />));
}
