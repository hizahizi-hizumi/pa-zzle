...
import { findSudokuSolution } from "@/games/sudoku/game/solver";
...
describe("useSudokuGame", () => {
  test("盤面完成後に完成状態を見せてから結果表示へ進めること", () => {
    const { result } = renderHook(() => useSudokuGame("normal"));
    const solution = findSudokuSolution(result.current.board);
