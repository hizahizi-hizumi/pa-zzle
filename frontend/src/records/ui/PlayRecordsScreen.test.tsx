...
describe("PlayRecordsScreen", () => {
  ...
  test("比較指標を列として揃えて自己ベストの文字列を履歴へ追加しないこと", () => {
    const personalBest = within(
      screen.getByRole("region", { name: "自己ベスト" }),
    );
    const playRecords = within(
      screen.getByRole("region", { name: "プレイ記録" }),
    );
    const replayButtons = playRecords.getAllByRole("button", {
      name: "同じ問題をプレイ",
    });

    expect(personalBest.getByText("93点")).toBeTruthy();
    expect(personalBest.getByText("-00:04")).toBeTruthy();
    expect(personalBest.getByText("±0")).toBeTruthy();
    expect(playRecords.queryByText("ベスト")).toBeNull();
    expect(replayButtons).toHaveLength(2);
    expect(playRecords.getByText("2件")).toBeTruthy();
  });
  ...
  test("指標の推移を折れ線グラフへ切り替えて確認できること", () => {
    ...
    const trendButton = playRecords.getByRole("tab", { name: "推移" });

    fireEvent.mouseDown(trendButton, { button: 0, ctrlKey: false });
    const metricSelect = playRecords.getByRole("combobox", {
      name: "推移する指標",
    });
    fireEvent.change(metricSelect, { target: { value: "time-delta-ms" } });
    const chart = playRecords.getByRole("img", {
      name: "基準時間との差の推移",
    });
  ...
  test("ゲームを切り替えるとゲーム固有の比較指標へ切り替わること", () => {
    ...
    expect(personalBest.getByText("01:30")).toBeTruthy();
    expect(personalBest.getByText("ミス")).toBeTruthy();
    expect(playRecords.getByText("1件")).toBeTruthy();
  });
});
